...
var GoogleMaps = {
    ...
    makeMarker: function(coordinates, /*locationStr,*/ title, url, description) {
        ...
        if (url) {
            content += ' <a href="' + url + '">mehr &raquo;<'+'/a>';
        }
        return {
            content: content,
...
GoogleMaps.map.prototype = {
    ...
    setGoogleMapOptions: function() {
        this.googleMapOptions = {
            ...
            center: this.mapConfig.coordinates ? GoogleMaps.str2coords(this.mapConfig.coordinates) : new google.maps.LatLng(52.522, 13.407),
    ...
    setMap: function() {
        ...
        if (this.mapConfig.center_active === true && this.mapConfig.coordinates) {
            var content = '';
            if (this.mapConfig.url) {
                content += '<a href="' + this.mapConfig.url + '">';
                if (this.mapConfig.alt) {
                    content += this.mapConfig.alt; 
                }   
                else {
                    content += this.mapConfig.locationStr;
                }   
                content += '</a>';
            }   
        ...
        for (var i = 0; i < this.mapConfig.markerList.length; ++i) {
            var marker = this.mapConfig.markerList[i],
                markerObj = GoogleMaps.makeMarker(
                    marker.coordinates, /*marker.address,*/ marker.title, marker.url, marker.description
                );
            if (false !== markerObj) {
